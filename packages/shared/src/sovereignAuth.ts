import { decodeRelayJwt } from "./relayJwt.ts";
import * as Option from "effect/Option";
import * as Schema from "effect/Schema";

export const SOVEREIGN_TOKEN_STORAGE_KEY = "t3code:sovereign-oauth-token:v1";
...
function readToken(storage: Storage): StoredToken | null {
  const encoded = storage.getItem(SOVEREIGN_TOKEN_STORAGE_KEY);
...
export function makeSovereignAuthClient(
  ...
  const saveToken = (token: StoredToken) => {
    dependencies.tokenStorage.setItem(SOVEREIGN_TOKEN_STORAGE_KEY, encodeStoredToken(token));
    return token;
  };

  const clear = () => {
    dependencies.tokenStorage.removeItem(SOVEREIGN_TOKEN_STORAGE_KEY);
  ...
  const completeSignInOnce = async (callbackUrl: string): Promise<string> => {
    const transactionEncoded = dependencies.transactionStorage.getItem(
      SOVEREIGN_TRANSACTION_STORAGE_KEY,
    );
