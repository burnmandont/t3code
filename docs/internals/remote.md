...
Tailscale, SSH, and Sovereign Relay change how the client reaches that server; they do
...
A client may retain multiple routes for the same stable environment ID. Selecting
Sovereign Relay or SSH changes only the transport; it must not duplicate projects,
threads, or subscriptions. Candidate routes are prepared before handoff so a failed
switch does not interrupt the working route.
...
[Sovereign Relay trust boundary](./t3-connect.md).
...
The server's `server-runtime.json` descriptor is a single-owner lease. Activation
fails while another live process owns it, and shutdown removes it only when it still
names that exact runtime. This prevents service discovery or SSH launch from starting
a second server against the same T3 home.
