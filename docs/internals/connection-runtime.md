...
One environment may retain several routes, such as Sovereign Relay, direct pairing,
and desktop-managed SSH. The catalog persists all known routes but selects only one
active target per client. Route changes prepare and authenticate the candidate before
replacing the active supervisor; a failed candidate leaves the existing session and
selection intact. Disconnect is temporary, while forgetting removes the local catalog
entry and its credentials.
