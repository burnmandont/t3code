...
This guide is for people who want to use more than one Claude setup in Sovereign. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Sovereign](./install.md).
...
In Sovereign Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Sovereign uses Claude Code's normal config directory.

When you set this field, Sovereign points Claude Code at that directory with the
...
Sovereign looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Sovereign looks.

Then add another Claude provider in Sovereign:
...
Sovereign only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Sovereign keeps separate config directories isolated
...
Add or edit a Claude provider in Sovereign Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Sovereign stores the value as a server secret and does not
...
Sovereign does not need a special Claude Code Router provider. Treat the router as a Claude
