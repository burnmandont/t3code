# Running Sovereign in the Background

On Linux and macOS, Sovereign can run as a background service for your user, so it is ready without
keeping a terminal open.

## Manage the Service

Install it with the latest Sovereign release:
...
Updating restarts Sovereign briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The service runs a small stable launcher. Exact Sovereign versions are installed separately, so a
...
## Using It with Sovereign Relay

Sovereign Relay may offer to install the service during setup so the host stays reachable in the
background. This is only an onboarding shortcut: the service and Sovereign Relay are managed separately.

Signing out of Sovereign Relay does not remove the service. Use `t3 service uninstall` when you no longer
want Sovereign to start in the background.
