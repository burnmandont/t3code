...
/**
 * Drives the primary environment's Sovereign Relay link. Sovereign Relay (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Sovereign Relay access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update Sovereign Relay", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Sovereign Relay",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Sovereign Relay off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Sovereign Relay before enabling this."));
        return false;
      }
