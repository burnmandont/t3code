        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          {environment.connection.error && !resumingServerUpdate ? (
            <div className="min-w-0 text-destructive text-xs">
              <p>{connectionStatusTitle(environment.connection)}</p>
              <p className="flex min-w-0 items-center gap-2">
                <span className="min-w-0 break-words">Reason: {environment.connection.error}</span>
                {errorTraceId ? (
                  <button
                    type="button"
        ...
        <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end">
          {routes.length > 1 ? (
            <Select
              ...
              onValueChange={(value) => {
                if (typeof value === "string") onSelectRoute(environmentId, value);
              }}
            >
              <SelectTrigger
                size="sm"
          ...
          (serverUpdateState.status === "idle" || serverUpdateState.status === "failed") ? (
            <ServerUpdateAction
              ...
              targetVersion={serverVersionNotice.clientVersion}
              label={
                serverUpdateState.status === "failed" ? "Retry" : runtimeDrift ? "Sync" : "Update"
              }
            />
          ) : null}
          {isWslEnvironment ? (
                ...
                control={
                  ...
                  primaryServerUpdateState.status !== "running" ? (
                    <ServerUpdateAction
                      ...
                      targetVersion={primaryServerVersionNotice.clientVersion}
                      label={
                        primaryServerUpdateState.status === "failed"
                          ? "Retry"
                          : primaryRuntimeDrift
