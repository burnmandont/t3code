...
/**
 * Settings access for global surfaces. Hosted clients have no primary target,
 * so use their active (or sole) remote environment when one is unambiguous.
 */
...
export function usePrimarySettingsAvailable(): boolean {
  const environmentId = useSettingsEnvironmentId();
  return environmentId !== null || !isHostedStaticApp();
}
