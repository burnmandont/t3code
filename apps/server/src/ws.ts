...
import * as RelayClient from "@t3tools/shared/connectorClient";
        ...
        [ORCHESTRATION_WS_METHODS.subscribeThread]: (input) =>
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              const liveCoalescer = yield* makeThreadLiveEventCoalescer();
              const liveBuffer = yield* makeThreadLiveBuffer<OrchestrationThreadStreamItem>(
                input.threadId,
              );
              const synchronizedBuffered = yield* Deferred.make<boolean>();
              yield* Effect.forkScoped(liveStream.pipe(Stream.runForEach(liveCoalescer.offer)));
              yield* Effect.forkScoped(
                liveCoalescer.stream.pipe(
                  Stream.runForEach((item) =>
                    offerThreadLiveItem(liveBuffer, item).pipe(
                      Effect.tap((offered) =>
                        item.kind === "synchronized"
                          ? Deferred.succeed(synchronizedBuffered, offered)
                          : Effect.void,
                      ),
                      Effect.flatMap((offered) => (offered ? Effect.void : Effect.interrupt)),
                    ),
                  ),
                  Effect.ensuring(Deferred.succeed(synchronizedBuffered, false)),
                ),
              );
              const bufferedLiveStream = Stream.fromQueue(liveBuffer.queue);
              const synchronizeLiveBuffer = liveCoalescer
                .offerAndWait({ kind: "synchronized" as const })
                .pipe(Effect.andThen(Deferred.await(synchronizedBuffered)), Effect.asVoid);
              ...
              if (input.afterSequence !== undefined) {
                ...
                if (replayGap >= 0 && replayGap <= THREAD_RESUME_MAX_GAP) {
                  ...
                  const afterCatchUp =
                    input.requestCompletionMarker === true
                      ? Stream.concat(
                          Stream.fromEffect(synchronizeLiveBuffer).pipe(Stream.drain),
                          bufferedLiveStream,
                        )
                      : bufferedLiveStream;
