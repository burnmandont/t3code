/**
 * All text `sovereign triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Sovereign triage playbook

You are a support engineer for Sovereign (https://github.com/pingdotgg/t3code), working
...
- How is Sovereign running on this machine: \`sovereign serve\` in a terminal, the
...
Do not patch the Sovereign source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Sovereign triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Sovereign user is \
having a problem with their install and started this session with \`sovereign triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Sovereign triage context

Generated by \`sovereign triage\` at ${input.generatedAt}.
