...
import type * as RelayClient from "@t3tools/shared/connectorClient";
...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored Sovereign Relay credential could not be refreshed; signing in again.",
...
export function formatCloudStatus(
  ...
  const nextStep = status.retired
    ? "This immutable environment identity cannot reconnect. Reinstall into a fresh Sovereign home to create a new identity."
    : !status.authenticated
      ? `Run \`${canonicalCliCommand("connect link")}\` to authorize and enable Sovereign Relay.`
      : !status.desired
        ? `Run \`${canonicalCliCommand("connect link")}\` to enable Sovereign Relay.`
        : !status.linked
          ? "Start Sovereign to provision the environment link and launch its managed tunnel."
          : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The T3 relay client is required for Sovereign Relay. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Sovereign Relay disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Sovereign Relay is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Sovereign Relay is disabled locally.");
    }
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : `Could not revoke the relay-side environment record yet.\nRun \`${canonicalCliCommand("connect unlink")}\` again when the relay is reachable.`,
      );
...
export const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      `Signed out of Sovereign Relay locally.\nThe background service is managed separately with \`${canonicalCliCommand("service")}\`.`,
    );
  }
});
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log("Sovereign Relay setup cancelled. The relay client was not installed.");
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Sovereign Relay CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("Sovereign Relay\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Sovereign Relay on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start Sovereign to publish agent activity (no managed tunnel).`
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Sovereign Relay and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only Sovereign Relay link.");
          }
        ...
        // out of band without Sovereign Relay.
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            `Run \`${canonicalCliCommand("connect login")}\` first so this environment can be authorized to publish.`,
          );
          return;
        }
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Sovereign Relay link is already pending. Start Sovereign to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Sovereign Relay while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Sovereign Relay and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up Sovereign Relay for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nSovereign is set to run while you are logged in to this Mac. The server establishes the Sovereign Relay link on startup."
              : "\n✓ Background service ready\n\nSovereign is set to keep running after you log out. The server establishes the Sovereign Relay link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
