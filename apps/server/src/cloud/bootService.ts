...
export const BOOT_SERVICE_OBSERVABILITY_ENV = "%h/.config/t3code/observability.env";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    `EnvironmentFile=-${BOOT_SERVICE_OBSERVABILITY_ENV}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Sovereign background service.";
  }
}
