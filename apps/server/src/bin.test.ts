...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.include(
        ...
        "missing Sovereign Relay public configuration",
      );
  ...
  it.effect("exposes service lifecycle commands without Sovereign Relay configuration", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Manage the Sovereign background service.");
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Sovereign Relay\n  Exposure: disabled");
      assert.include(output, "  Authorization: missing");
      assert.include(output, "  Environment link: not provisioned");
      assert.include(
        output,
        "Next: Run `t3 connect link` to authorize and enable Sovereign Relay.",
      );
    }),
  );
  ...
  it.effect("disables headless connect without a running server", () =>
    Effect.gen(function* () {
      ...
      assert.equal(output, "Sovereign Relay is disabled locally.");
    }),
  );
