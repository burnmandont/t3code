...
import { useMobileCloudAuth } from "./CloudAuthProvider";
import { hasCloudPublicConfig } from "./publicConfig";

/**
 * Post-sign-in onboarding sheet for Sovereign Relay. Mobile never publishes
 * environments itself — it consumes ones published elsewhere — so this simply
 * surfaces the account's Sovereign Relay environments right after sign-in so every
...
function ConfiguredConnectOnboardingRouteScreen() {
  ...
  const { isSignedIn, userId } = useMobileCloudAuth();
        ...
        {isSignedIn ? (
          ...
          <View collapsable={false} className="rounded-[24px] bg-card p-5">
            <Text className="text-sm leading-normal text-foreground-muted">
              Sign in to your Sovereign account to set up Sovereign Relay.
            </Text>
          </View>
        )}
