...
describe("electron development launcher", () => {
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/Sovereign (Dev).app",
      "Sovereign (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "Sovereign (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Sovereign (Dev).app/Contents/MacOS/Sovereign (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Sovereign (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/Sovereign (Dev).app/Contents/MacOS/Electron'",
    );
  ...
  it("derives launcher icons from canonical development and production assets", () => {
    ...
    assert.match(development.sourceIconPath, /assets\/dev\/sovereign-dev-macos-1024\.png$/);
    assert.equal(development.generatedIconPath, "/runtime/icon-dev.icns");
    assert.match(production.sourceIconPath, /assets\/prod\/sovereign-macos-1024\.png$/);
