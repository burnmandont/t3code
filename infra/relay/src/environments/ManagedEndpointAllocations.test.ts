...
describe("ManagedEndpointAllocations", () => {
  it.effect("lists only allocations without an active managed link", () => {
    ...
    const fakeDb = {
      select: () => ({
        from: (table: unknown) => {
          ...
          return {
            leftJoin: (joined: unknown, condition: unknown) => {
              expect(joined).toBe(relayEnvironmentLinks);
    ...
    return Effect.gen(function* () {
      ...
      expect(
        yield* allocations.listOrphaned({ updatedBefore: "2026-08-08T15:45:00.000Z" }),
      ).toEqual([]);

      const dialect = new PgDialect();
      const join = dialect.sqlToQuery(joinCondition as never);
      expect(join.sql).toContain(
        '"relay_environment_links"."user_id" = "relay_managed_endpoint_allocations"."user_id"',
      );
      expect(join.sql).toContain(
        '"relay_environment_links"."environment_id" = "relay_managed_endpoint_allocations"."environment_id"',
      );
      expect(join.sql).toContain('"relay_environment_links"."revoked_at" is null');
      expect(join.sql).toContain('"relay_environment_links"."managed_tunnels_enabled" = $1');
      expect(join.params).toEqual([true]);

      const where = dialect.sqlToQuery(whereCondition as never);
      expect(where.sql).toContain('"relay_environment_links"."user_id" is null');
      expect(where.sql).toContain('"relay_managed_endpoint_allocations"."updated_at" < $1');
      expect(where.params).toEqual(["2026-08-08T15:45:00.000Z"]);
