    ...
    expect(
      loadPublicClientProvisioningConfiguration({
        ...
        T3_ACCOUNT_CLIENT_REDIRECT_URIS:
          "https://app.example.test/oauth/callback,t3code://oauth/callback,https://app.example.test/oauth/callback",
        T3_ACCOUNT_CLIENT_POST_LOGOUT_REDIRECT_URIS: "t3code://signed-out",
      }),
    ).toEqual({
      clientId: "sovereign-t3",
      name: "T3 Code",
      redirectUris: ["https://app.example.test/oauth/callback", "t3code://oauth/callback"],
